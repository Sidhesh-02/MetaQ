...
export default function WorkPage() {
  ...
  return (
    <main className="relative py-20 w-full min-h-screen bg-white overflow-hidden">
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-6 grid gap-8 sm:grid-cols-2">
        {images.map((src, i) => (
          <div
            key={i}
            className="group relative rounded-2xl border border-zinc-200  bg-white  shadow-md overflow-hidden cursor-pointer transition transform hover:-translate-y-1 hover:shadow-2xl"
            onClick={() => setSelected(src)}
          >
            {/* Card Title */}
            <div className="relative px-4 py-2 border-b border-zinc-100  text-sm font-medium text-zinc-700  backdrop-blur-sm">
              Case Study {i + 1}
            </div>
      ...
      {selected && (
        ...
        >
          <div className="relative w-[95%] max-w-5xl h-[85%] bg-white  rounded-3xl p-4 shadow-xl">
            <Image
              src={selected}
              alt="Selected Graph"
              fill
